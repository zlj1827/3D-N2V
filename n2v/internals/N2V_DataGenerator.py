...
class N2V_DataGenerator():
    ...
    def __extract_patches__(self, data, num_patches=None, shape=(256, 256), n_dims=2):
        if num_patches == None:
            ...
            if n_dims == 2:
                ...
                for i in range(num_patches):
                    z, y, x = np.random.randint(0, data.shape[1] - shape[0] + 1), np.random.randint(0,data.shape[2] - shape[1] + 1), np.random.randint(
    ...
    ## Data augmentation is performed on the patch.
