...
class N2V_DataWrapper(RollingSequence):
    ...
    def __getitem__(self, i):
        ...
        ## Replace the blind spot pixels
        for c in range(self.n_chan):
            for j in range(self.batch_size):

                coords = self.get_stratified_coords(self.rand_float, box_size=self.box_size,
                                                    shape=self.shape) # Select the blind spot location (multiple)

                indexing = (j,) + coords + (c,)
                indexing_mask = (j,) + coords + (c + self.n_chan, )

                y_val = self.X_Batches[indexing] # The raw pixel value is used as the target
                x_val = self.value_manipulation(
                    self.X_Batches[j, ..., c],
                    coords,
                    self.dims,
                    self.structN2Vmask
                ) # Replace the pixel value in the blind spot and use it as the network input

                self.Y_Batches[indexing] = y_val # target
                self.Y_Batches[indexing_mask] = 1 # mask
    ...
    ## Uniform coordinate generating function (avoid concentration of blind spots)
    @staticmethod
